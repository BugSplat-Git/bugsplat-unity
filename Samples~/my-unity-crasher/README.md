...
- **The sample turns on symbol upload for every platform.** `UploadDebugSymbolsFor` Windows, macOS, iOS and Android are all checked on the sample's `BugSplatOptions` asset, so a player build uploads symbols and the dashboard shows symbolicated C# and native stacks — which is most of the point of running these scenarios. Symbol upload needs credentials, and they are deliberately not stored on the asset: set them once per database with **BugSplat > Symbol Upload > Set Credentials**, or with the `SYMBOL_UPLOAD_CLIENT_ID` / `SYMBOL_UPLOAD_CLIENT_SECRET` environment variables. Without them the build **warns and still succeeds**, so you get a player either way — just an unsymbolicated one. Windows additionally needs **Copy PDB Files** in Build Settings and a Windows editor. See [Symbol Upload](../../Documentation~/symbol-upload.md).
