...
| --- | --- | --- |
| `MANAGED` | The .NET handler, via `Application.logMessageReceived`: unhandled exception, coroutine exception, caught & posted manually, background-thread exception (a documented gap — no report today) | All |
...
- **The status line tells you what to expect.** On Windows it reports whether the WER handler is armed. When it isn't, the `FAIL-FAST` rows are greyed out and say so, because running them would terminate the player and report nothing. Register the handler with **BugSplat > Windows > Register WER Handler**, or see [Windows Error Reporting](../../../README.md#windows-error-reporting).
