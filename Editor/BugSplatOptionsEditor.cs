...
namespace BugSplatUnity.Editor
{
    ...
    public class BugSplatOptionsEditor : UnityEditor.Editor
    {
        ...
        private const string hangDialogConflictMessage = "Apple Auto Submit Fatal Hang Report is off, but Apple Auto Submit Crash Report is on, so fatal hangs will still upload without asking. Turn Apple Auto Submit Crash Report off as well to be prompted.";
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (!options.AppleAutoSubmitFatalHangReport && options.AppleAutoSubmitCrashReport)
            {
                EditorGUILayout.HelpBox(hangDialogConflictMessage, MessageType.Warning);
            }
