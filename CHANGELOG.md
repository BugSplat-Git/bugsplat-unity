...
- `BugSplatOptions` is grouped into Inspector sections — **BugSplat Account**, **Report Metadata**, **Capture**, **Apple (iOS and macOS)**, **iOS**, **macOS**, **Android**, **Windows** — instead of presenting 26 fields as one flat list. The Apple section sits before iOS and macOS so the two options that apply to both are findable from either; they previously sat inside the macOS block and read as macOS-only. Field names are unchanged, so existing options assets keep their values.
- `BugSplatOptions.AppleAutoSubmitCrashReport` and `BugSplatOptions.AppleAutoSubmitFatalHangReport` — control whether Apple crash and fatal hang reports are submitted without asking the user. Named after the bugsplat-apple properties they map to, and shared by iOS and macOS rather than split per platform. `AutoSubmitFatalHangReport` defaults to `true`, matching the SDK; turning it off routes a hang through the same dialog a crash shows, and needs `AutoSubmitCrashReport` off too. Hang dialogs need a `BugSplat-macOS.dylib` carrying `autoSubmitFatalHangReport`; against an older dylib the option logs a notice and does nothing.
- **bugsplat-apple updated to 3.6.0** (from 3.5.0), which brings the macOS crash dialog fixes and the reporting fix behind `AppleAutoSubmitFatalHangReport`:
  ...
  - `autoSubmitFatalHangReport` exists, so `BugSplatOptions.AppleAutoSubmitFatalHangReport` is now live rather than logging a notice and doing nothing.
...
- **Breaking:** iOS native crash reports are no longer auto-submitted by default. The iOS bridge hard-coded `autoSubmitCrashReport = YES`, so a crash uploaded without asking; it is now driven by `BugSplatOptions.AppleAutoSubmitCrashReport`, which defaults to `false` and shows the BugSplat dialog on the next launch — matching macOS and Windows, which both prompt by default. Set `AppleAutoSubmitCrashReport` to `true` to restore silent uploads.
- **Breaking:** the `BugSplat` constructor gained `autoSubmitCrashReport` and `autoSubmitFatalHangReport` parameters after `capturePlayerLog`. Both are optional, so only code passing arguments positionally past `capturePlayerLog` needs changing.
