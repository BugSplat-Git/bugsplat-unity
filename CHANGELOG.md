...
- **bugsplat-apple updated to 3.6.0** (from 3.5.0), which brings the macOS crash dialog fixes and the reporting fix behind `AutoSubmitFatalHangReport`:
  ...
  - `autoSubmitFatalHangReport` exists, so `BugSplatOptions.AutoSubmitFatalHangReport` is now live rather than logging a notice and doing nothing.
...
- **Breaking:** iOS native crash reports are no longer auto-submitted by default. The iOS bridge hard-coded `autoSubmitCrashReport = YES`, so a crash uploaded without asking; it is now driven by `BugSplatOptions.AutoSubmitCrashReport`, which defaults to `false` and shows the BugSplat dialog on the next launch — matching macOS and Windows, which both prompt by default. Set `AutoSubmitCrashReport` to `true` to restore silent uploads.
