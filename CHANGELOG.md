...
- The iOS and macOS native bridges now export the same symbol names instead of `Ios`/`Mac`-suffixed pairs. If you imported the sample before this change, delete the imported copy and re-import it: samples are copied into `Assets/` and do not update with the package, so a stale copy fails the IL2CPP link with an undefined `__crashNativeMac`. See [Migrating from 4.x](Documentation~/migrating-from-4x.md). The two plugins are gated to mutually exclusive platforms and can never be compiled into the same binary, so the suffixes bought nothing while forcing every Apple call site in `BugSplat.cs` to be written twice; eight duplicated `#if UNITY_IOS … #elif UNITY_STANDALONE_OSX` branches and one whole `DllImport` block collapsed as a result. These symbols are internal P/Invoke targets, so no public API changed.
