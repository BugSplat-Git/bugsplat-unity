...
- Builds **fail** when no options asset is selected, when several exist and none is selected, when the selected asset's database is empty with Initialize Automatically on, or when a *different* `BugSplatOptions` sits in **Player Settings > Preloaded Assets** — a player takes its options from the first one loaded, so two would make the choice depend on load order. A misconfigured project previously built a player that silently reported nothing.
...
- **bugsplat-apple updated to 3.6.1** (from 3.6.0). 3.6.1 fixes stacked crash reports on macOS and iOS: when more than one report is pending — the app crashed again before one could upload, or it was offline — approving the dialog once now sends the rest silently instead of showing a dialog per report. Declining still discards them all, as before. 3.6.0 brought the macOS crash dialog fixes and the reporting fix behind the auto-submit hang options:
...
- iOS builds failed to compile with `Cannot use '@try' with Objective-C exceptions disabled`. `BugSplatBridge.mm` guards its `NSFileHandle` reads with `@try`, and Unity's `UnityFramework` target sets `GCC_ENABLE_OBJC_EXCEPTIONS = NO`. The plugin importer now passes `-fobjc-exceptions` for that file, and the Xcode post-process sets the same flag on the file in the generated project, covering projects that imported the plugin before the importer setting existed. Only if the file cannot be found there does it fall back to enabling `GCC_ENABLE_OBJC_EXCEPTIONS` for the whole `UnityFramework` target.
- Native macOS crash reports carried the wrong `Player.log`. A report uploads at the next launch, and by then Unity has renamed the crashed session's log to `Player-prev.log` and started a fresh `Player.log` — which is what was attached, so the log contained nothing about the crash. The bridge now reads `Player-prev.log` for that report; when the SDK provides the crashed session ID it verifies the file identity before attaching it, and otherwise it attaches on best effort. If the app crashes again before BugSplat initializes, Unity's rotation has already replaced that log; the report then carries no `Player.log` rather than the wrong one.
