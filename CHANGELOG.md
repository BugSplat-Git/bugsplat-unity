...
- **bugsplat-apple updated to 3.6.1** (from 3.5.0). 3.6.1 fixes stacked crash reports on macOS and iOS: when more than one report is pending — the app crashed again before one could upload, or it was offline — approving the dialog once now sends the rest silently instead of showing a dialog per report. Declining still discards them all, as before. 3.6.0 brought the macOS crash dialog fixes and the reporting fix behind the auto-submit hang options:
