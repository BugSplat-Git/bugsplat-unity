...
- Builds **fail** when no options asset is selected, when several exist and none is selected, or when the selected asset's database is empty with Initialize Automatically on. A misconfigured project previously built a player that silently reported nothing.
