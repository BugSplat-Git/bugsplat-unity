...
- Native macOS crash reports carried the wrong `Player.log`. A report uploads at the next launch, and by then Unity has renamed the crashed session's log to `Player-prev.log` and started a fresh `Player.log` — which is what was attached, so the log contained nothing about the crash. The bridge now reads `Player-prev.log` for that report, and verifies it is the crashed session's file before attaching it. If the app crashes again before BugSplat initializes, Unity's rotation has already replaced that log; the report then carries no `Player.log` rather than the wrong one.
